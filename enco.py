...
# --- メイン処理部（エラーハンドリングと対話型入力） ---
def process_features_interactive():
    print("=== 特徴量抽出処理スクリプト ===")

    # 1. 入力ファイル名の受付けとチェック
    while True:
        input_path = input("入力ファイル名（例: train1.xlsx）を入力してください: ").strip()
        if not input_path:
            print("⚠️ ファイル名が入力されていません。もう一度入力してください。")
            continue

        if not os.path.exists(input_path):
            print(f"❌ エラー: 指定されたファイル '{input_path}' が見つかりません。パスを確認してください。\n")
        else:
            break
    ...
    while True:
        output_path = input(
            "保存先の出力ファイル名（例: train_features.xlsx）を入力してください: ").strip()
        if not output_path:
            print("⚠️ 保存先のファイル名を入力してください。")
            continue
        if not output_path.endswith('.xlsx'):
            output_path += '.xlsx'  # 拡張子の補正
        break
    ...
    try:
        ...
        print(
            f"✅ 保存完了: '{output_path}' (データ数: {out_df.shape[0]}行, カラム数: {out_df.shape[1]}列)")

    except Exception as e:
        print(f"❌ 計算または保存処理中にエラーが発生しました: {e}")


# 実行
if __name__ == "__main__":
    process_features_interactive()
